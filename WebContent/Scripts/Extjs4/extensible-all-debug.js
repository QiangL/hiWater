...
Extensible.calendar.data.EventMappings = {
    ...
    HidePreTimeInViewItem: {
        name: 'HidePreTimeInViewItem',
        mapping: 'hidePreTimeInViewItem',
        type: 'boolean'
    }
};/**
...
Ext.define('Extensible.calendar.gadget.CalendarListPanel', {
    ...
    onClick : function(e, t){
        ...
        if(el = e.getTarget(this.menuSelector, 3, true)){
            this.showEventMenu(el, e.getXY());//-------------------------------------------------------sinoyd
        }
        ...
        var items = [{
            ...
            listeners: {//----------------------------------------------------------------------------sinoyd
                select: function (field, value, options) {
                    field.up().items.items[1].items.items[1].setValue(value[0].data.StartWork);
                    field.up().items.items[1].items.items[3].setValue(value[0].data.EndWork);
                    }
            }
        }, {
    ...
    */
    ...
    onCalendarEndDrag: function (start, end, onComplete) {
        ...
        if (this.fireEvent('rangeselect', this, dates, onComplete) !== false) {
            this.showEventEditor(dates, null); //----------------------------------------------------------------------sinoyd
            if (this.editWin) {
    ...
    */
    ...
    onDayClick: function (dt, ad, el) {
        ...
        if (this.fireEvent('dayclick', this, Ext.Date.clone(dt), ad, el) !== false) {
           ...
           // this.showEventEditor(data, el); //--------------------------------------------------------------sinoyd
        }
    },
    ...
    showEventMenu: function (el, xy) {
        if (!this.eventMenu) {
            this.eventMenu = Ext.create('Extensible.calendar.menu.Event', {
                listeners: {
                //    'editdetails': Ext.bind(this.onEditDetails, this),------------------------------------------------------sinoyd
    ...
    onClick: function (e, t) {
        ...
        if (el) {
            ...
            if (me.fireEvent('eventclick', me, rec, el) !== false) {
                if (me.readOnly !== true) {
                   // me.showEventEditor(rec, el); //----------------------------------------------------------sinoyd
                }
            }
            return true;
        }
...
//        data.Title = (evt[M.IsAllDay.name] ? '' : Ext.Date.format(evt[M.StartDate.name], fmt)) + 
        ...
        data.Title = ((evt[M.HidePreTimeInViewItem.name] || evt[M.IsAllDay.name]) ? '' : Ext.Date.format(evt[M.StartDate.name], fmt)) +
                (!title || title.length == 0 ? this.defaultEventTitleText : title);
